...
export default function ListingsPage({ category, condition, location }: Props) {
  ...
  const initialPage = searchParams
    ? parseInt(searchParams.get("paged") || "1", 10)
    : 1;
  ...
  useEffect(() => {
    // Check if searchParams is not null before accessing it
    if (searchParams) {
      ...
      // If searchParams is null, you can log an error or set default behavior
      console.log("searchParams is null, using default page 1");
      loadListings(1, filtersRef.current); // Optionally load the first page if no searchParams
    }
  ...
  const updateURLWithFilters = (page: number) => {
    ...
    if (searchParams) {
      ...
      // Loop through the filters and update the URL with them
      ...
      // Update the URL without triggering the page reload
      router.push(finalUrl);
    } else {
      // If searchParams is null, handle the case appropriately
      console.error("searchParams is null, cannot update the URL.");
      // Optionally, you could set a default URL or handle it however you want
      router.push(`${buildSlugPath()}?paged=1`);
    }
  };
