...
const CaravanFilter: React.FC<CaravanFilterProps> = ({
  ...
  useEffect(() => {
    if (selectedSuggestion) setLocationInput(selectedSuggestion.short_address);
  }, [selectedSuggestion]);
  useEffect(() => {
    if (
      selectedSuburbName &&
  ...
  const justSelectedRegionRef = useRef(false);
  // 1) NEW state
  const [searchQuery, setSearchQuery] = useState("");

  // 2) REPLACE the old locationInput effect with this:
  useEffect(() => {
    const q = searchQuery.trim();
    const t = setTimeout(() => {
      if (q.length >= 2) {
        fetchLocations(q).then(setLocationSuggestions).catch(console.error);
      } else {
  ...
  console.log("yy loc", searchQuery);
  ...
  useEffect(() => {
    if (
      justSelectedStateRef.current &&
      selectedStateName &&
      !selectedRegionName
    ) {
      setStateOpen(true); // 🔥 open region list
  ...
  useEffect(() => {
    if (
      justSelectedRegionRef.current &&
      selectedRegionName &&
      !selectedSuburbName &&
      filteredSuburbs.length > 0
    ) {
      setStateOpen(true); // 🔥 open suburb list
      justSelectedRegionRef.current = false;
    }
  }, [selectedRegionName, selectedSuburbName, filteredSuburbs]);
                ...
                ?.regions?.map((region, idx) => (
                  <div
                    ...
                    onClick={() => {
                      ...
                      justSelectedRegionRef.current = true;
                    ...
                    return (
                      <li
                        ...
                        onClick={() => {
                          ...
                          setSearchQuery(item.short_address); // optional: mirror in search box
                          setLocationSuggestions([]);
                          suburbClickedRef.current = true;
                        }}
                      >
